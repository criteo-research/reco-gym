...
from recogym.constants import AgentStatsSale
...
def _collect_stats_sale(args):
    ...
    if with_cache:
        ...
        with tqdm(total=data.shape[0], desc='Offline Logs') as pbar:
            for _, row in data.iterrows():
                pbar.update()
                t, u, z, v, a, c, r, ps, ps_a = row.values
    ...
    rewards = stat_data[~np.isnan(stat_data['a'])]['r']
    clicks = stat_data[~np.isnan(stat_data['a'])]['c']
    successes = np.sum(rewards)
    failures = np.sum(rewards == 0)
    successes_click = np.sum(clicks)
    failures_click = np.sum(clicks == 0)
    print(f"END: Agent Evaluating @ Epoch #{epoch} ({time.time() - start}s)")

    return {
        AgentStatsSale.SUCCESSES: successes,
        AgentStatsSale.FAILURES: failures,
        AgentStatsSale.SUCCESSES_click: successes_click,
        AgentStatsSale.FAILURES_click: failures_click
    }
...
def test_agent_sale(
    ...
    for result in [_collect_stats_sale(args) for args in argss]:
        successes += result[AgentStatsSale.SUCCESSES]
        failures += result[AgentStatsSale.FAILURES]
        successes_click += result[AgentStatsSale.SUCCESSES_click]
        failures_click += result[AgentStatsSale.FAILURES_click]

    return  ((beta.ppf(0.500, successes + 1, failures + 1), 
            beta.ppf(0.025, successes + 1, failures + 1),
            beta.ppf(0.975, successes + 1, failures + 1)),
            (beta.ppf(0.500, successes_click + 1, failures_click + 1), 
            beta.ppf(0.025, successes_click + 1, failures_click + 1),
            beta.ppf(0.975, successes_click + 1, failures_click + 1))
    )